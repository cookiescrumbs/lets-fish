...
namespace :waters do
    ...
    task :add_waters_from_forsinard_file do
        ...
        waters.each do |water|
            ...
            body = {
                ...
                water_type_id: 1,
                species_ids: [1],
                address: 'South Uist, Scotland'
                }
            }
            begin
                response = HTTParty.post('https://lets.fish/api/water/fishery/', body: body.to_json, headers: { 'Content-Type' => 'application/json' })
                p response
                p body
            rescue StandardError => e
                p e
    ...
    desc 'Clean waters and add waters from json file'
    task :add_waters_from_south_uist_estate_file do
    waters = JSON.parse(File.read(Dir.pwd + '/lib/tasks/json_files/south-uist-estate.json'))
        waters.each do |water|
            location = NorthernPike.os_reference_to_lat_lng(os_ref: "NF" + water['lat_lng'])
            ...
            begin
              response = HTTParty.post('https://lets.fish/api/water/fishery/south-uist-estate', body: body.to_json, headers: { 'Content-Type' => 'application/json' })
              p response
              p body
            rescue StandardError => e
              p e
